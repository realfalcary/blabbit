...
def init_db():
    with get_db() as db:
        ...
        try: db.execute('ALTER TABLE users ADD COLUMN status_emoji TEXT DEFAULT NULL')
        except: pass
        try: db.execute('ALTER TABLE users ADD COLUMN status_text TEXT DEFAULT NULL')
        except: pass
        db.commit()
...
def update_status():
    ...
    with get_db() as db:
        db.execute('UPDATE users SET status_emoji=?, status_text=? WHERE username=?',
                   (emoji or None, text or None, caller))
        db.commit()
    return jsonify({'ok': True})
...
def profile(username):
    caller = session.get('username')
    with get_db() as db:
        row = db.execute('SELECT username,created,pronouns,about_me,banner,profile_color1,profile_color2,status_emoji,status_text FROM users WHERE username=? COLLATE NOCASE', (username,)).fetchone()
    ...
    return jsonify({'ok':True,'username':row['username'],'created':row['created'],
                    ...
                    'status_emoji': row['status_emoji'] or '', 'status_text': row['status_text'] or ''})
...
@app.route('/api/servers/<int:server_id>/icon', methods=['POST'])
def update_server_icon(server_id):
    caller = session.get('username')
    if not caller: return jsonify({'ok': False}), 401
    with get_db() as db:
        server = db.execute('SELECT owner FROM servers WHERE id=?', (server_id,)).fetchone()
        if not server: return jsonify({'ok': False, 'error': 'Not found'}), 404
        if server['owner'] != caller: return jsonify({'ok': False, 'error': 'Not owner'}), 403
        icon = request.get_json().get('icon')
        db.execute('UPDATE servers SET icon=? WHERE id=?', (icon, server_id))
        db.commit()
    # Push updated server list to all members
    with get_db() as db:
        members = db.execute('SELECT username FROM server_members WHERE server_id=?', (server_id,)).fetchall()
    for sid, info in online_users.items():
        if any(m['username'] == info['username'] for m in members):
            socketio.emit('servers_update', {'servers': get_user_servers(info['username'])}, to=sid)
    return jsonify({'ok': True})
...
def server_members(server_id):
    ...
    with get_db() as db:
        server = db.execute('SELECT owner FROM servers WHERE id=?', (server_id,)).fetchone()
        if not server: return jsonify({'ok': False}), 404
        rows = db.execute('SELECT username FROM server_members WHERE server_id=?', (server_id,)).fetchall()
    online = {info['username'] for info in online_users.values()}
    members = [{'username': r['username'], 'online': r['username'] in online,
                'is_owner': r['username'] == server['owner']} for r in rows]
    members.sort(key=lambda m: (not m['is_owner'], not m['online'], m['username'].lower()))
    return jsonify({'ok': True, 'members': members, 'owner': server['owner']})
...
if __name__ == '__main__':
    ...
    try:
        avatars.update(load_all_avatars())
        print(f"[STARTUP] avatars loaded: {len(avatars)}", flush=True)
    except Exception as e:
        print(f"[STARTUP] load_all_avatars() FAILED: {e}", flush=True)
